...
from sampler import sampler_dict, sample_pdf_rays
...
def render_images(
    ...
    for cam_idx, camera in enumerate(cameras):
        ...
        if cam_idx == 0 and file_prefix == '':
            ...
            plt.imsave('images/xy_vis.png', xy_vis)
        ...
        if cam_idx == 0 and file_prefix == '':
            render_points('images/sample_points.png', ray_bundle.sample_points.reshape(1, -1, 3), device=device)
        ...
        if fine_model:
            ...
            ray_bundle_fine = sample_pdf_rays(
                z_vals_coarse=out['depth_values'].detach(),
                weights=out['weights'].detach().squeeze(-1),
                ray_bundle=ray_bundle,
                N_samples=128
            )
            # ray_bundle_fine = model.sampler(ray_bundle)
            # Run the fine model
            out = fine_model(ray_bundle_fine, is_sample=False)
        ...
        if cam_idx == 2 and file_prefix == '':
            depth = np.array(out['depth'].view(
                    image_size[1], image_size[0]
                ).detach().cpu())
            plt.imsave('images/depth_map.png', depth)
...
def train_nerf(
    ...
    if cfg.training.fine_network:
        fine_model, fine_optimizer, fine_lr_scheduler, _, _ = create_model(cfg)

    # Load the training/validation data.
    train_dataset, val_dataset, _ = get_nerf_datasets(
        dataset_name=cfg.data.dataset_name,
        image_size=[cfg.data.image_size[1], cfg.data.image_size[0]],
        limit_view=cfg.training.limit_view
    )
        ...
        for iteration, batch in t_range:
            ...
            if cfg.training.fine_network:
                ...
                ray_bundle_fine = sample_pdf_rays(
                    ...
                    weights=out['weights'].detach().squeeze(-1),
                    ray_bundle=ray_bundle,
                    N_samples=cfg.sampler.n_fine_pts_per_ray
                )
                
                # ray_bundle_fine = model.sampler(ray_bundle)
                out = fine_model(ray_bundle_fine, is_sample=False)
                loss_fine = torch.nn.functional.mse_loss(out['feature'], rgb_gt)
                loss = loss + loss_fine
        ...
        if (
            ...
            with torch.no_grad():
                ...
                test_images = render_images(
                    ...
                    cfg.data.image_size, file_prefix='nerf', fine_model=fine_model if cfg.training.fine_network else None
                )
                if cfg.training.limit_view:
                    imageio.mimsave('images/part_8_2_2.gif', [np.uint8(im * 255) for im in test_images], loop=0)
                elif cfg.training.fine_network:
                    imageio.mimsave('images/part_4_2.gif', [np.uint8(im * 255) for im in test_images], loop=0)
                elif cfg.implicit_function.use_view_directions:
                    imageio.mimsave('images/part_4_1.gif', [np.uint8(im * 255) for im in test_images], loop=0)
                else:
