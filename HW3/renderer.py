...
class VolumeRenderer(torch.nn.Module):
    ...
    def _compute_weights(
        ...
        alpha = 1.0 - torch.exp(-deltas * rays_density)
        t = 1.0 - alpha
        t = torch.cat([torch.ones((t.shape[0], 1, 1)).cuda(), t[:, :-1]], axis=1)
        transmittance = torch.cumprod(t, axis=1)
    ...
    def _aggregate(
        ...
        feature = torch.sum(weights * rays_feature, axis=1)
    ...
    def forward(
        ...
        for chunk_start in range(0, B, self._chunk_size):
            ...
            # TODO (1.5): Render (color) features using weightss
...
class SphereTracingRenderer(torch.nn.Module):
    ...
    def sphere_tracing(
        ...
        for i in range(self.max_iters):
            v = implicit_fn(res)
            res = res + v * directions
            
            mask = v.squeeze(1) < epsilon
            if torch.all(mask):
                break

        return res, mask.unsqueeze(1)
...
def sdf_to_density(signed_distance, alpha, beta):
    ...
    density = alpha * torch.where(signed_distance>0, 0.5*torch.exp(-signed_distance/beta), 1 - 0.5 * torch.exp(signed_distance/beta))
    return density
...
class VolumeSDFRenderer(VolumeRenderer):
    def __init__(
        ...
        if "use_neus" in dict(self.cfg):
            self.use_neus = self.cfg.use_neus
            self.s = self.cfg.s
            # print("Use Neus sdf_to_density")
