...
def render(
    ...
    if cfg.implicit_function.sdf.type == 'combined':
...
def train_images(
    ...
    for epoch in range(start_epoch, cfg.training.num_epochs):
        ...
        if (
            ...
            if 'use_neus' in dict(cfg.renderer) and cfg.renderer.use_neus:
                imageio.mimsave(f'images/part_8_3.gif', [np.uint8(im * 255) for im in test_images], loop=0)
            elif cfg.training.limit_view:
                imageio.mimsave(f'images/part_8_2_0.gif', [np.uint8(im * 255) for im in test_images], loop=0)
            else:
                imageio.mimsave(f'images/part_7_a{cfg.renderer.alpha}_b{cfg.renderer.beta}.gif', [np.uint8(im * 255) for im in test_images], loop=0)
            ...
            try:
                ...
                if 'use_neus' in dict(cfg.renderer) and cfg.renderer.use_neus:
                    imageio.mimsave(f'images/part_8_3_geometry.gif', [np.uint8(im * 255) for im in test_images], loop=0)
                elif cfg.training.limit_view:
                    imageio.mimsave(f'images/part_8_2_0_geometry.gif', [np.uint8(im * 255) for im in test_images], loop=0)
                else:
                    imageio.mimsave(f'images/part_7_geometry_a{cfg.renderer.alpha}_b{cfg.renderer.beta}.gif', [np.uint8(im * 255) for im in test_images], loop=0)
            except Exception as e:
                print("Empty mesh")
                pass
