...
def optimize_mesh_texture(
    ...
    R, T = look_at_view_transform(dist=3, elev=0, azim=np.linspace(-180, 180, 10, endpoint=False))
    ...
    for i in tqdm(range(total_iter)):
        ...
        rend = renderer(mesh, cameras=random.choice(query_cameras), lights=lights)
        rend = rend.permute(0, 3, 1, 2)[:, :3, :, :]
