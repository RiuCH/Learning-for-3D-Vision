...
class SDS:
    ...
    def sds_loss(
        ...
        with torch.no_grad():
            ...
            latent_noise = self.scheduler.add_noise(latents, noise, t)

            noise_pred = self.unet(latent_noise, t, encoder_hidden_states=text_embeddings).sample

            if text_embeddings_uncond is not None and guidance_scale != 1:
                ### YOUR CODE HERE ###
                noise_pred_uncond = self.unet(latent_noise, t, encoder_hidden_states=text_embeddings_uncond).sample
        ...
        loss = F.mse_loss(latents.float(), (latents + grad).detach(), reduction='sum') / latents.shape[0]
