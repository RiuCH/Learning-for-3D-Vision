...
class Gaussians:
    ...
    def _load_random(self, num_points: int):
        ...
        data["spherical_harmonics"] = torch.randn((num_points, 48), dtype=torch.float32) * 0.1  # (N, 48)
    ...
    def compute_cov_3D(self, quats: torch.Tensor, scales: torch.Tensor):
        ...
        if self.is_isotropic:
            ...
            ### YOUR CODE HERE ### 
            diag_vals = (scales * scales).expand(-1, 3)  # (N, 3)
            cov_3D = torch.diag_embed(diag_vals)  # (N, 3, 3)
        ...
        # HINT: You can use a function from pytorch3d to convert quaternions to rotation matrices.
            ...
            cov_3D = torch.bmm(RS, torch.transpose(RS, 1, 2))  # (N, 3, 3)
    ...
    def compute_cov_2D(
        ...
        cov_2D = torch.bmm(JW, torch.bmm(cov_3D, torch.transpose(JW, 1, 2)))  # (N, 2, 2)
    ...
    def evaluate_gaussian_2D(points_2D: torch.Tensor, means_2D: torch.Tensor, cov_2D_inverse: torch.Tensor):
        ...
        product1 = torch.matmul(dist, cov_2D_inverse)  # (N, H*W, 2)
        quadratic_term = (product1 * dist).sum(dim=-1)  # (N, H*W)
        power = -0.5 * quadratic_term  # (N, H*W)
        return power
...
class Scene:
    ...
    def compute_depth_values(self, camera: PerspectiveCameras):
        ...
        mean3d = self.gaussians.means  # (N, 3)
        z_vals = camera.get_world_to_view_transform().transform_points(mean3d)[:, 2]  # (N,)
        return z_vals
    ...
    def get_idxs_to_filter_and_sort(self, z_vals: torch.Tensor):
        ...
        """
        ### YOUR CODE HERE ###
        sorted_z_vals, sorted_indices = torch.sort(z_vals)
        valid_mask = sorted_z_vals >= 0
        idxs = sorted_indices[valid_mask]
        return idxs
    ...
    def compute_alphas(self, opacities, means_2D, cov_2D, img_size):
        ...
        ### YOUR CODE HERE ###
        # HINT: Refer to README for a relevant equation.
        alphas = opacities.unsqueeze(1).expand(-1, H*W) * exp_power  # (N, H*W)
    ...
    def calculate_gaussian_directions(self, means_3D, camera):
        ...
        gaussian_dirs = F.normalize(means_3D - cam_pos, dim=-1)
