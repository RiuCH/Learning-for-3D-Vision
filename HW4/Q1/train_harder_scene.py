...
# from utils import l1_loss, ssim_loss
...
def ssim_loss(pred, target, window_size=11):
    ...
    sigma_x_sq = F.avg_pool2d(pred * pred, window_size, stride=1, padding=window_size//2) - mu_x_sq
    sigma_y_sq = F.avg_pool2d(target * target, window_size, stride=1, padding=window_size//2) - mu_y_sq
...
def setup_optimizer(gaussians):
    ...
    parameters = [
        ...
        {'params': [gaussians.spherical_harmonics], 'lr': 0.0025/20, "name": "spherical_harmonics]"},
    ]
...
def run_training(args):
    ...
    for itr in range(args.num_itrs):
        ...
        # Compute loss
        ### YOUR CODE HERE ###
        loss = 0.8 * l1_loss(pred_img, gt_img) + 0.2 * (1.0 - ssim_loss(pred_img, gt_img))
